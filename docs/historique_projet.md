...
- **2026-06-15.6** : Refonte structurelle majeure de l'écosystème utilisateur. Mise en œuvre d'une navigation sémantique responsive latérale à sélecteurs d'onglets (Dashboard, Mes Formulaires, Nouveau Projet, Paramètres). Intégration d'un magasin local de projets avec moteur de recherche textuel asynchrone, options interactives de duplication profonde (clonage autonome des questionnaires) et de suppressions physiques sécurisées. Implémentation d'un widget interactif de quotas journaliers et de curseurs de mise à niveau de comptes dans les réglages système.
